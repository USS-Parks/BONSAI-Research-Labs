...
def main() -> None:
    ...
    assert authority.name.startswith("bonsai-bx11-"), authority
    batch = ROOT / "target" / ("bx11-live-" + str(time.time_ns()))
    batch.mkdir()
    base = json.loads((ROOT / "fixtures/experiment-manifest/v1/valid.json").read_text())
    spec = json.loads((ROOT / "fixtures/external-environment/v1/spec.json").read_text())
        ...
        for horizon in [2, 8]:
            for name, identity, version, module, config, touches in [
                ...
                ("linear", "bonsai-linear-nlms", "1.0.0", "linear_adapter",
                 {"action_count": 4, "observation_width": spec["observation_width"]}, spec["observation_width"] + 1),
            ]:
                case = batch / f"{name}-s{seed}-h{horizon}"
                case.mkdir()
                manifest = json.loads(json.dumps(base))
                manifest["environment"]["config"]["horizon"] = horizon
                manifest["scenario"]["config"]["horizon"] = horizon
                manifest["adapter"] = {
                ...
                row = {"agent": name, "seed": seed, "horizon": horizon, "case": case.relative_to(ROOT).as_posix(),
    ...
    result = {"schema": "bonsai.optional-environment-runs/v1",
