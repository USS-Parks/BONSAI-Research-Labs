...
for record in records:
    if record["prompt"].startswith("BX-11"):
        for stream in ["stdout", "stderr"]:
            ...
            outputs += 1
archive_record = next(record for record in reversed(records) if record["prompt"] == "BX-11-ARCHIVE")
archive = json.loads(blob("evidence/verification/" + archive_record["stdout"]["path"]))
data = blob("evidence/verification/bx-11/external-environment.zip")
