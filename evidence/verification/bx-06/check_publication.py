...
for record in records:
    if record["prompt"].startswith("BX-06"):
        for stream in ["stdout", "stderr"]:
            ...
            outputs += 1
archive_record = next(record for record in reversed(records) if record["prompt"] == "BX-06-ARCHIVE")
archive = json.loads(blob("evidence/verification/" + archive_record["stdout"]["path"]))
data = blob("evidence/verification/bx-06/final-live.zip")
