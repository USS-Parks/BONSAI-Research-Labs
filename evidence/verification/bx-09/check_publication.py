...
for record in records:
    if record["prompt"].startswith("BX-09"):
        for stream in ["stdout", "stderr"]:
            ...
            outputs += 1
archive_record = next(record for record in reversed(records) if record["prompt"] == "BX-09-ARCHIVE")
archive = json.loads(blob("evidence/verification/" + archive_record["stdout"]["path"]))
data = blob("evidence/verification/bx-09/compatibility.zip")
