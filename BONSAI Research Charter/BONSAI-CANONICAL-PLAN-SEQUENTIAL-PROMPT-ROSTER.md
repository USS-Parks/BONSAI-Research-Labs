...
- [ ] **BK-01 — Versioned metric registry and engine.** **Depends:** BC-08, BC-11, BR-05. **Files:** `bonsai-metrics`, `docs/metrics/REGISTRY.md`. **Objective:** register metric ID/version/formula/unit/window/direction/inputs/availability/claim use and compute derived tables deterministically. **Excludes:** unregistered report calculations. **Gate:** dependency DAG, cycle detection, version pinning, and golden fixtures pass across OS.
- [ ] **BK-02 — Primary behavior metrics.** **Depends:** BK-01. **Files:** behavior metric modules/specs. **Objective:** cumulative reward/rate, regret when defensible, lifelong AUC, competency time, recovery, worst-window, and age-conditioned behavior. **Excludes:** regret without a valid comparator. **Gate:** analytical toy curves have exact expected results, units, windows, and unavailable states.
- [ ] **BK-03 — Resource and overhead metrics.** **Depends:** BK-01, BM-04. **Files:** resource metric modules. **Objective:** summarize CPU/wall/accelerator/memory/storage/I/O/work/energy, budget headroom, violations, sampling error, and paired instrumentation overhead. **Excludes:** cross-platform aggregation of incomparable fields. **Gate:** calibrated workloads reproduce known totals within declared error and enforce D-11 ceilings.
...
- [ ] **BE-01 — Scenario protocol and deterministic fixture framework.** **Depends:** BR-06, BC-03. **Files:** scenario adapter contract, `scenarios/`, docs. **Objective:** define observations/actions/reward/termination/change points, diagnostic truth, big-world generation, seeds, stream identity, and privileged diagnostic channel. **Excludes:** exposing privileged truth to Track A agents. **Gate:** same seed produces identical semantic stream across OS; privileged data is observer-only and forces Track D if exposed.
