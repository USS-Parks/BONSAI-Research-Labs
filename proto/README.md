...
[`bonsai/adapter/v1/adapter.proto`](bonsai/adapter/v1/adapter.proto) is the BR-01 process-protocol authority. `AdapterFrame` carries independent per-peer sequence numbers, negotiated epoch/minor, an immutable capability fingerprint, and exactly one start/handshake/configure/reset/step/work/feedback/event/stop message or corresponding result. The normative state machine, deterministic-seed/deadline semantics, capability and track facts, and explicit non-sandbox boundary are specified in [`../docs/architecture/ADAPTER-PROTOCOL.md`](../docs/architecture/ADAPTER-PROTOCOL.md).
