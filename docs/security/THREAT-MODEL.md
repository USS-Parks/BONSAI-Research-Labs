...
| TB-07 | Secrets | Inventory field removal; `xtask --redact`; publication secret scan; no raw environment dumps | `crates/bonsai-xtask/src/main.rs`; `crates/bonsai-xtask/src/schema_check.rs`; `crates/bonsai-accept/tests/m4_acceptance.rs` | Redaction is defense in depth. Operators must not pass secrets on command lines. | BC-04, BG-04, BG-06, BV-12, BV-16 |
