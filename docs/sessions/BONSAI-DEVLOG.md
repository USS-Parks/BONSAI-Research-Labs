...
- Status: passed locally and on hosted public Actions; focused commit `a57ee5b` is on `main`
...
- Commit SHA: `a57ee5b5d170ff7249e4a6533022ea0da10a4628`
- Hosted verification: https://github.com/USS-Parks/BONSAI-Research-Labs/actions/runs/32756387561
...
- Reuse classification: compose existing ingest, bundle, storage, recovery, and inventory controls in `bonsai-accept`; add operator docs and RC evidence
- Files changed: threat model, hardening suite, dependency policy/SBOM/offline scripts, L harness and not-run attestations, operator handoff, RC notes/audit/hashes, roster/README honesty, DEVLOG, verification log, machine records
...
### Prompt evidence index (M4)

| ID | Gate evidence |
|---|---|
| BV-11 | `docs/security/THREAT-MODEL.md` + `bv11_threat_model_covers_every_trust_boundary` |
| BV-12 | HMAC/redaction/bounds + flood/tamper/recovery suite |
| BV-13 | lockfile hashes, empty waivers, SBOM/offline scripts; physical clean-machine not-run |
...
### BX-04 checkout portability correction

Initial main commit `d3b2087ef7c85d3a670c840c3953cfb249e8cb46` passed local gates and all committed hashes. A post-publication checkout-index probe with core.autocrlf=true reproduced CRLF conversion in both generated Python binding files, breaking the intentional byte-for-byte regeneration gate. Two explicit eol=lf rules fix checkout behavior without changing generated bytes or weakening verification. A small corrective commit preserves published history; BX-04 remains open until that revision passes hosted CI.

### BX-04 final closeout (recorded by BX-05)

- Corrected main/origin/main: `9c15015a5c6252c5c461f1e13b6be154e48d1c9a`; hosted [run 34002423080](https://github.com/USS-Parks/BONSAI-Research-Labs/actions/runs/34002423080) fully successful across all four platforms and semantic equivalence.
- Source commit d3b2087ef7c85d3a670c840c3953cfb249e8cb46 plus a focused checkout-policy correction; superseded run 34002336855 was cancelled. No history rewrite.
- Six committed output hashes and 21 source hashes independently verified. Windows CRLF checkout probe now preserves both generated binding files exactly. BX-04 complete; BX-05 begins.
...
### BX-05 failure-boundary checkpoint — 2026-09-06T01:41:26.058Z

BX-05 remains executing and uncommitted. The long runner functions were split into manifest, report, child launch/cleanup, action exchange, feedback, WorkMeter, and resource helpers. Strict Linux Clippy now passes without added suppressions. The full Windows Python suite passes all 59 tests. The live Linux bounded-transport regression passes with 20 repeated cleanups, threads 3->3 and FDs 5->5; write/read timeout causes survive an explicit shutdown after containment, while cancellation during a pending shutdown still returns cancellation.

Live failure evidence is retained in `target/bx05-failures-1788658289489622537`, `target/bx05-failures-1788658607548660695`, and `target/bx05-failures-1788658774340906252`. In the second matrix, child exit, agent storage exhaustion, late response, and cancellation during recorded action execution passed their exact expected INCOMPLETE classifications. The early memory fixtures hit the 50 ms deadline before the memory cap: both had zero OOM kills, so neither is OOM proof. After moving the bounded 256 MiB allocation into adapter initialization under a 64 MiB cgroup limit, the focused memory case passed `AGENT_RESOURCE_VIOLATION`; the retained counters show two OOM-kill events and 42 memory-max events, with both owned groups cleaned up. Preserve all failed attempts; do not relabel them as passed.

Source identity is checked before and after execution, including all Rust/Python/protocol/schema/build/lock files and the operator binary; component entrypoint hashes are also compared before/after. A 256 KiB heap hashing buffer bounds memory and reduces small-read overhead. Source metadata work still adds roughly 24 seconds per diagnostic on the Windows-mounted Linux checkout. The successful 2,000-step S diagnostic from the preceding checkpoint predates these final identity/refactor changes and must be rerun after final packaging.

Next work: finish canonical seven-role result-bundle packaging and meaningful complete/incomplete metric metadata; make observer-output quota and report/finalization failures unambiguously INCOMPLETE; verify the full wall deadline after provenance/finalization and consider a cancellation watchdog for actual in-flight enforcement; make the resolved Linux CPU/memory/task controls explicit in immutable inputs rather than relying on undocumented defaults; test source/component drift and unsupported-manifest rejection; rerun final S plus the complete failure matrix on frozen source; refresh Cargo.lock-dependent SBOM/RC checksums, run all publication gates, archive byte-verified evidence, commit/push main, and wait for hosted matrix. BX-06 C0/C1 remains unstarted. No test/build process remains active at this checkpoint.


### BX-05 finalization and metadata checkpoint — 2026-09-06T01:48:49.662Z

BX-05 remains executing and uncommitted. A durable, atomically replaced `observer/run-status.json` now begins as INCOMPLETE before execution. Errors during run finalization preserve INCOMPLETE with `RUN_FINALIZATION_FAILED`; successful execution is explicitly distinguished from completed bundle finalization in report metadata. The report-write fault test in `target/bx05-failures-1788659088111930980/finalization` passed: a directory occupying the HTML output path caused a real write error, operator exit 1, and authoritative INCOMPLETE status. The test initially rejected its new case because the harness allowlist omitted it; that harness error was corrected before the passing run.

Evidence now accumulates reward count/sum only after EventIngestor accepts the corresponding reward event. The report-write-failure fixture retained 20 reward events totaling 9; its report independently matched both numbers. Strict Linux Clippy passes for these changes.

Added a build script recording the actual compiler version supplied by Cargo and a Linux inventory collector for CPU information, guest-visible memory/page size, measured monotonic-clock resolution, lockfile hashes, compiler identity, and explicit unavailable capabilities. Inventory IDs are run-local, with no physical-host or cross-run identity claim. The collector is wired into output/report generation and passes Clippy, but its live/schema acceptance still remains.

Remaining BX-05 work: assemble and validate all seven canonical result-bundle roles, including actual resource policy, pending track declaration, failure log, and truthful metric/uncertainty files; retain complete input/source hashes; enforce total observer-output quota and verify final wall deadline/cancellation behavior; resolve explicit immutable control declarations; test source/component drift and unsupported manifests; perform final frozen-source S and failure gates; refresh SBOM/RC lockfile hashes; run full Windows/Linux/publication checks, archive immutable evidence, commit/push main, and wait for required hosted CI. No active process remains at this checkpoint.

## BX-05 — local acceptance passed; hosted publication pending

Date: 2026-09-06 UTC. Authorization: the user's full BX-01–BX-40 STS instruction. Source baseline: `9c15015a5c6252c5c461f1e13b6be154e48d1c9a`. BX-05 remains in progress until its focused commit is published and required hosted CI passes; BX-06 has not started.

Implemented the Linux `cargo xtask run` seam by composing the existing gated child transport, delegated cgroup authority, primitive controller, causal environment, governor accounts, ingestor, segment writer, lifecycle journal, and report generator. Added shared ordered Python adapter lifecycle plus typed primitive reward/accounting payloads. The runner records immutable inputs and resolved controls before launch, measures actual action/resource/work behavior, verifies source/component identity before and after execution, enforces pending-operation cancellation, and retains authoritative INCOMPLETE state through finalization failures. Metadata/output allowances and an artifact index bound all observer files. Absolute executable identity is mandatory. C0/C1 and derived track facts remain BX-06.

Final live batch: `target/bx05-failures-1788660837367174365`. All 11 cases passed. S completed 2,000 steps in 56.327551905 seconds end-to-end (54.219912705 seconds through execution/provenance finalization), with 966 finished episodes, reward sum 957, 8,000 measured work items, and 35,822 events. Independent reconciliation found exactly 2,000 actions, 2,000 rewards, 8,000 admission/charge work events, and 2,000 resource samples; maximum action latency 2,681,421 ns, maximum per-step agent CPU 1,734,000 ns, total observer bytes 17,530,327, and 18 indexed artifacts. All declared S bounds passed. Report generation remains outside action deadlines.

Failure facts were independently reconciled: forced child exit 23; actual kernel OOM counters (2 kills / 36 memory-max events); 65,537 agent bytes against 65,536; delayed response 54,955,096 ns against 50 ms; cancellation after real actions; cancellation after initialization entry while the child was blocked; expired-before-launch wall rejection; observer exhaustion with 137,059 retained bytes within the 262,144-byte cap; and report-write failure retaining INCOMPLETE. Every case's owned child groups were empty after cleanup. Five real preflight negatives rejected wrong revision, wrong patch hash, unsupported profile, unsupported required counter, and a relative executable without creating output.

Historical failed attempts remain preserved. `BX-05-LIVE-FAILURES-1788660482727413000` failed because an already-classified wall failure returned exit 1 rather than 2. Independent auditing also found that its combined pending-cancellation case reused the prior case's marker and never entered initialization. Unique per-case markers, an explicit initialization-entry assertion, and corrected wall finalization handling resolved both; the final full matrix and independent audit passed. Earlier metadata scope-coverage failures, transport classification fixes, and preliminary S runs remain documented above and in retained target diagnostics.

Verification: 212 Windows Rust tests and 214 Linux Rust tests passed; all 59 Python tests passed on both systems. Formatting, strict Clippy, Ruff, Pyright, schemas, documentation/ADR/license/risk/terminology/CI governance, and M4 honesty checks passed. Cargo.lock-dependent SBOM/RC hashes were refreshed using LF output. This is local Windows plus WSL2 Linux evidence, not physical-Linux or hosted-CI proof.
...
## BX-09 — live compatibility accepted; full Linux/publication pending

The existing protocol now supports minors 1.0 and 1.1, pins the selected version for every later frame, and refuses unsupported epochs/minors even inside broad offers. Minor 1.1 adds bounded required/optional capability-name lists under the existing immutable fingerprint. Unknown required capabilities and unsupported checkpoint restore fail closed; optional hints confer no authority. The existing seven-check conformance suite and generic runtime remain the execution seam.

Preserved the previous generated binding/proto from BX-08 main and a complete trusted historical reference-source set. The governed verifier accepts a complete supported current or historical set; archived positive and negative governed-bundle tests pass. A new breaking-epoch schema fixture requires migration/refusal. Source-level and direct-dependency audit found no reference-agent imports in runtime, governor or platform authority.

Formal live evidence: BX-09-LIVE-WINDOWS-1788671461367358000 and BX-09-LIVE-LINUX-1788671480107121000 each passed 12 actual child-process certifications (two versions, three seeds, two trials), with launch audits, durable lifecycle journals, recorded exchanges, deterministic action digests and real post-handshake timeout containment. Both hosts produced report SHA-256 `3a0938ee9bafc3415d8977269dda72af4be0a53bf8a26accce97a443b1281ee2`. These are contract fixtures, not scientific-quality or hostile-host certification.

Archive `evidence/verification/bx-09/compatibility.zip`: 537,499 bytes, SHA-256 `c6eb34c2f45de2f4e7c28079aebfc46679d785fa0daee927c59e8e75baf00320`; all 273 payloads re-read and checked, including 180 exact source files. The archived harness snapshot precedes import-formatting-only cleanup; product source identity is unchanged.

Windows full gate BX-09-WINDOWS-1788671625036769600 passed 240 Rust / 59 Python tests and all lint, schema, generated-binding, docs and governance checks. Linux full gate remains in progress; final closeout, byte check, focused main commit/push and hosted CI remain pending. An early new version-pinning regression exposed a remaining range check; it was corrected and the test now passes. An initial probe compile issue and evidence-harness import formatting were corrected before final acceptance. Diagnostic target/bx09-live-windows-initial remains preserved.

Storage at 2026-09-06T05:16:03.149Z: canonical target/.venv measured at least 31,129,403,857 bytes (31 unreadable entries), retained for the ongoing approved STS; 325,768,048,640 bytes available. Historical M0 is clean, zero unpublished commits, 1,463,283,025 generated bytes, no active task, retained cleanup debt pending explicit removal authorization. No worktree or cache deletion. BX-10 remains unstarted.
...
## BX-10 implementation checkpoint — governed smoke pair passed

BX-09 baseline is `4b6ef2f9871f83e24f283dafc0c7f6f0f5fa8b40`. Selected an original fixed-size linear NLMS contextual reward predictor after reviewing existing adapters/comparator declarations and Vowpal Wabbit licensing/build scope; rationale and primary sources are in `docs/operator/second-learner-selection.md`. No new dependency or third-party source was imported.

Extracted OnlineAdapter/OnlineControl so tabular and linear factories share the same lifecycle, observation/action/feedback/accounting behavior. The linear model uses a fixed action-by-feature matrix and normalized squared-error updates; both models record bounded before/after parameter values in an additive PrimitiveAccounting field. Existing primitive behavior tests pass; independent linear numeric/rejection/storage tests and shared live adapter tests pass; Pyright is clean.

Added a versioned optional component accounting declaration, generic counter/update shape/action/reward validation, and replaced the runner's fixed two-touch assumption. The independent scientific verifier supports the two explicitly pinned implementations while preserving complete BX-08 and BX-09 historical source sets. Three accounting refusal/history tests and seven archived governed-bundle tests pass.

Diagnostic governed batch `target/bx10-live-1788673665377626553` completed both 20-step seed-42 runs using the same operator SHA-256 `e5421882cf050e010bdc371df5eca8043a3f98add9509b294d9bca5cb7fcaf7d`, identical custom resource profiles, and the same causal-ring world. Both independent verifiers returned C0/C1 pass and Track A, with existing E0/virtualized/hostile-host limitations. Tabular receipt `7ecd4fc2208de8ffdd4920861bcef85ac01e05278d52f53650ca19cbc0fba600`; linear receipt `883bb350e8944cda2c1654d17c40927a206802a112099a4061dcb368276a91cd`. This is diagnostic smoke evidence; final source may change.

Remaining: independently reconstruct all recorded numeric updates, collect actual per-agent timeout and seven-check conformance evidence, run five paired seeds with determinism trials under one fixed operator/source identity, retain separate resources and negative accounting cases, archive final sources/results, full Windows/Linux/governance/byte gates, focused main commit/push and all hosted jobs. BX-11 remains unstarted; no new worktree or deletion.

### BX-10 verification checkpoint — full-trace conformance correction

The first final matrix (`target/bx10-live-1788674303814192812`) completed all 20 governed 200-step runs and both real timeout probes. `BX-10-LIVE-PAIRED-1788674223270887600` passed. Numeric replay confirmed distinct tabular and linear updates and repeatability. Full-trace certification initially exceeded its 60-second diagnostic deadline because the existing partial-order classifier repeatedly searched an already total-ordered event chain. That process also held the Windows executable while the concurrent build attempted to link it; the captured Windows gate failed with LNK1104. The Linux gate independently caught a 103-line function lint. These failures remain recorded.

The ordering checker now recognizes a proven contiguous single-source total order and skips redundant cycle/concurrency searches while preserving all diagnostics; backward causal edges and ambiguous/gapped sources still use the general algorithm. Six ordering regressions pass, including a 4,000-event chain and negative duplicate/late/clock/missing-parent/gap/cycle cases. Work-meter construction now owns accounting-declaration parsing. `BX-10-REPLAY-DIAGNOSTIC-1788675315991034400` certifies all 20 retained traces (140 passing checks), and `BX-10-DEPENDENCIES-1788675369169679000` confirms no learner identity/import in authority or orchestration. Full regression retries and a fresh source-frozen final matrix are required before commit; BX-10 remains executing.

BX-10 full local regressions now pass: Windows `BX-10-WINDOWS-1788675446991058000`, Linux `BX-10-LINUX-1788675338388494900`. The corrected classifier also certified all 20 diagnostic traces. A fresh paired matrix will bind the final corrected source and supervisor binary before archive/commit; no scientific-quality or physical-host claim is made.
...
## 2026-09-06 — BX-13 post-publication portability correction

The original BX-13 source/evidence commit `82ed6a5b3eaa6f5eb36349df53edfd26dd44692e` was pushed normally to public main after SHIP review, with 81 committed files and 42 captures passing exact byte checks. During hosted artifact inspection, parent found that Windows checkout converted newly pinned source files to CRLF. Git's actual `core.autocrlf=true` checkout filters reproduced four byte mismatches: `brdc1.py`, `option_learning.py`, `option_adapter.py` and `chain_adapter.py`. Because the independent verifier embeds these bytes, this could make a Windows-built verifier reject a Linux-produced current-source receipt. This finding was not identified by the original local tests or SHIP review; BX-13 remains [~] and BX-14 has not started.

The focused correction pins those four runtime sources plus the hosted helper, option tests and workflow to LF, and adds a hosted CI guard derived from the verifier's actual source list. It is a justified additional BX-13 commit to repair a defect found after publication. The original 16,829,864-byte archive remains immutable at SHA-256 `030b972470cf36f7ea252180acce8226da28df6ef8f9b8af6bfb8cc73e1e631c`. No learner, adapter, Rust verifier, budget or runtime source byte changed; only checkout rules and CI evidence collection changed among the original 258 source files. The original governed matrix and learning evidence therefore remain the behavioral proof, with an explicit metadata-only source delta for this correction.

Retained `evidence/verification/bx-13-portability/checkout-before.json` and `checkout-after.json` show four mismatches before and none after across all 20 source pins. `original-windows-protocol.zip` retains GitHub artifact 9994859794 from run 34051838111, SHA-256 `c58b56d15a79fa9d2e9563cc8a5ffd09414429d60c3f8a0bfae847fd343f45ba`; its three source hashes exactly match CRLF conversion of their published LF blobs. Record `BX-13-PORTABLE-VERIFIER-1788719926544299200` passes actual native Windows verification of all 13 retained Linux governed bundles with their recorded receipt digests; complete results are in `native-verification.json`. This proves offline receipt portability on the current native verifier, without asserting new learner, controller or physical-host acceptance.

The CI worker is extending the existing helper in place. Parent verification, a fresh independent correction review, normal follow-up publication and all five jobs at the corrected commit are required before closing BX-13. No new worktree or deletion; previously disclosed storage retention and cleanup debt remain.


## 2026-09-06 — BX-13 portability parent verification

The correction is limited to `.gitattributes`, the existing hosted protocol helper and supplemental evidence/governance. Record `BX-13-PORTABLE-SOURCE-GUARD-1788720057651265700` passes all 20 current byte pins, rejects each of the four reproduced Windows CRLF conversions and rejects four missing-attribute cases. It verifies exactly 256 of the original 258 source files remain byte-identical; the two explicit deltas are checkout rules and CI evidence collection. The original governed archive is unchanged. `guard-verification.json` retains those exact source deltas.

Record `BX-13-PORTABLE-PROTOCOL-1788720064964061000` passes all 11 actual-process cases through the final helper. Its complete local source map, JUnit, outputs and summary are retained beside the native cross-host receipt check in `evidence/verification/bx-13-portability/`. The helper records 20 raw Git blobs, filtered blobs, checkout hashes and text/eol attributes. Hosted runs require byte equality, an empty tracked patch and GITHUB_SHA equal to HEAD; local dirty runs state their limits. A new source path not yet in HEAD is an explicitly bounded local-workflow limitation to handle when a later prompt adds such a path.

Astra result: the CI worker completed and froze the one-file correction on observed Sol/high (native call `exec-a66d7b48-5f61-4f22-beee-a8ad8ee3e58e`). Parent inspected the actual helper diff and reran its gates. API-EQUIVALENT COST RECEIPT: token and cost telemetry remain unavailable; coverage is partial, with no savings claim. Fresh independent correction review and corrected-commit hosted results remain pending.


Portability closeout gate note: `BX-13-PORTABLE-GOVERNANCE-1788720096574778200` initially failed only repository lint on formatting in the two new supplemental evidence scripts. Imports and layout were corrected; `BX-13-PORTABLE-GOVERNANCE-FINAL-1788720117374885600` passes full repository Ruff, docs, governance ledgers, terminology and M4 checks. Both attempts and exact captures are retained.


Original commit CI closeout: run [34051838111](https://github.com/USS-Parks/BONSAI-Research-Labs/actions/runs/34051838111) passed Windows x64, Linux x64, macOS ARM64, macOS x64 and M1 hosted semantic equivalence at `82ed6a5b3eaa6f5eb36349df53edfd26dd44692e`. The original run did not detect the checkout-identity gap; it is retained as original-commit evidence in `original-hosted-ci.json`, not substituted for corrected-commit CI. `BX-13-PORTABLE-PUBLICATION-SCAN-1788720244154150000` found no credential patterns in the staged correction or either retained archive. Final corrected attribute/source-delta proof is `BX-13-PORTABLE-SOURCE-GUARD-FINAL-1788720167739561200`; exact supplemental capture bytes are preserved by explicit Git attributes.


## 2026-09-06 — BX-13 portability correction SHIP

Fresh independent reviewer `/root/bx13_portability_review` returned SHIP with no blocking finding after parent verification. It inspected the final 34-file staged patch (reviewed binary patch SHA-256 `79910673d0dff69f0837fabbf384b84d41384ac491103ab07b520971697ab16f`), verified every source-list entry matches its embedded path and LF attributes, confirmed exactly two metadata deltas in the frozen source identity, checked all 13 native Windows receipt results, and verified local protocol JUnit/output hashes and the unchanged original archive. Parent accepts the correction. Final publication-byte checks passed 34 files and all 56 BX-13 captured outputs with zero mismatches.

The reviewer requested Sol/high; actual runtime and token telemetry were unavailable. Review made no file, index, commit, push or worktree changes. API-EQUIVALENT COST RECEIPT: partial and unavailable; no cost or savings estimate. Original hosted artifacts were also independently checked: 11 passing protocol cases on each of four platforms, correct original commit identity and archive/output hashes; the Windows artifact explicitly retains its known CRLF checkout conversion. Corrected-commit source cleanliness, byte equality and all five hosted jobs are still required. BX-13 remains [~] and BX-14 is unstarted until that closeout.
